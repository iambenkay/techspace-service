...
async function mustHaveRequirement(req, res, next) {
  ...
  const noDocumentReq = !Object.keys(business.requirements.document || {})
    .length;
  const noStatutoryReq = !Object.keys(business.requirements.statutory | {})
    .length;
  if (noDocumentReq && noStatutoryReq)
    return res.status(400).send({
      error: true,
