...
module.exports.retrieve = async request => {
    ...
    request.V.expr("The business is not tied to this vendor account", businesses.length)
    return new Response(200, {
        error: false,
        business: (({
            business: {
...
module.exports.destroy = async request => {
    ...
    if (!businessEmail) throw new ResponseError(400, "Business email was not provided")
    const business = await Account.find({ email: businessEmail })
    if (!business) throw new ResponseError(400, "You can't remove a non-existent business account")
    const isATiedBusiness = await Business_Vendor_Rel.find({ businessId: business.id, vendorId: id })
    if (!isATiedBusiness) throw new ResponseError(400, "Business is not tied to your vendor account")
